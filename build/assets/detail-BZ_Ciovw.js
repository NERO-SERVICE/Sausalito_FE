import{m as k,a as R,s as v,g as w,r as S,i as A,d as L,j as E,k as C,l as T,t as N,n as O}from"./footer-DLgmIPmE.js";import{a as y,c as Y}from"./cart-service-LKHAjlVQ.js";import{a as m,f as u}from"./store-data-Deux6dsM.js";const c=Number(new URLSearchParams(location.search).get("id")),t={product:null,meta:null,reviews:[],imageIndex:0,quantity:1,reviewPage:1,activeSection:"section-detail",open:{shipping:!1,inquiry:!1},policyOpen:!1,wished:!1},j=k({showCart:!0,currentNav:"shop"});R();const h={root:document.getElementById("detailRoot")};let d=null;const g=10;let l=null,p=!1;function B(){window.matchMedia("(min-width: 821px)").matches||(window.addEventListener("touchstart",a=>{window.scrollY>0||(l=a.touches[0].clientY,p=!1)},{passive:!0}),window.addEventListener("touchmove",a=>{if(l===null||p||window.scrollY>0)return;a.touches[0].clientY-l>90&&(p=!0,history.length>1?history.back():location.href="/pages/home.html")},{passive:!0}),window.addEventListener("touchend",()=>{l=null,p=!1},{passive:!0}))}async function $(){const a=await v()||w();let e=0;try{e=await Y()}catch{e=0}L(j,{userName:a?.name||a?.email||null,isAdmin:!!(a?.is_staff??a?.isStaff),cartCountValue:e})}function H(){const a=m(t.product.image,{useFallback:!1}),e=Array.isArray(t.product.images)?t.product.images:[],i=Array.isArray(t.meta?.detailImages)?t.meta.detailImages:[],s=[...new Set([a,...e,...i].filter(n=>typeof n=="string"&&n.trim()))];return s.length?s:[m(null)]}function U(){const a=Math.max(1,Math.ceil(t.reviews.length/g));t.reviewPage=Math.min(a,Math.max(1,t.reviewPage));const e=(t.reviewPage-1)*g;return{totalPages:a,list:t.reviews.slice(e,e+g)}}function W(a){t.activeSection=a,document.querySelectorAll(".pd-tab").forEach(e=>{e.classList.toggle("active",e.dataset.target===a)})}function F(){d&&d.disconnect();const a=[...document.querySelectorAll(".pd-section")];d=new IntersectionObserver(e=>{const i=e.filter(s=>s.isIntersecting).sort((s,n)=>n.intersectionRatio-s.intersectionRatio)[0];i?.target?.id&&W(i.target.id)},{threshold:[.3,.6],rootMargin:"-20% 0px -45% 0px"}),a.forEach(e=>d.observe(e))}function I(){if(!t.product){h.root.innerHTML='<p class="empty">상품을 찾을 수 없습니다.</p>';return}const a=H(),e=a.length>1;t.imageIndex=Math.min(a.length-1,Math.max(0,t.imageIndex));const i=a[t.imageIndex]||m(null),n=(t.meta.options&&t.meta.options[0]||{price:t.product.price}).price*t.quantity,P=Math.round((1-t.product.price/t.product.originalPrice)*100),x=t.reviews.length?(t.reviews.reduce((o,r)=>o+r.score,0)/t.reviews.length).toFixed(1):Number(t.product.rating||0).toFixed(1),{list:f,totalPages:b}=U();h.root.innerHTML=`
    ...
    <section class="pd-tabs">
      ...
      <section class="pd-section-card pd-section" id="section-review">
        ...
        ${f.length?f.map(o=>{const r=Array.isArray(o.images)&&o.images.length?o.images.slice(0,3):o.image?[o.image]:[];return`<article class="pd-review-item">
  ...
  `,F()}document.addEventListener("click",async a=>{const e=a.target.closest("[data-action]");if(!e)return;const i=e.dataset.action;if(i==="prevImage"&&(t.imageIndex-=1),i==="nextImage"&&(t.imageIndex+=1),i==="selectImage"&&(t.imageIndex=Number(e.dataset.index)),i==="increaseQty"&&(t.quantity=Math.min(99,t.quantity+1)),i==="decreaseQty"&&(t.quantity=Math.max(1,t.quantity-1)),i==="scrollTab"){const s=document.getElementById(e.dataset.target);s&&s.scrollIntoView({behavior:"smooth",block:"start"}),t.activeSection=e.dataset.target}if(i==="toggleOpen"){const s=e.dataset.key;t.open[s]=!t.open[s]}if(i==="prevReview"&&(t.reviewPage=Math.max(1,t.reviewPage-1)),i==="nextReview"&&(t.reviewPage+=1),i==="openPolicy"&&(t.policyOpen=!0),i==="closePolicy"&&(t.policyOpen=!1),i==="addCart")try{await y(t.product.id,t.quantity),alert("장바구니에 담았습니다."),await $()}catch(s){if(console.error(s),alert(s.message||"장바구니 담기에 실패했습니다."),s.status===401||s.message.includes("로그인")){location.href="/pages/login.html";return}}if(i==="buyNow")try{await y(t.product.id,t.quantity),location.href="/pages/cart.html";return}catch(s){if(console.error(s),alert(s.message||"장바구니 담기에 실패했습니다."),s.status===401||s.message.includes("로그인")){location.href="/pages/login.html";return}}if(i==="toggleWish"){if(!(await v()||w())){alert("로그인 후 이용 가능합니다."),location.href="/pages/login.html";return}try{t.wished?(await S(t.product.id),t.wished=!1,alert("위시리스트에서 제거했습니다.")):(await A(t.product.id),t.wished=!0,alert("위시리스트에 추가했습니다."))}catch(n){console.error(n),alert(n.message||"찜 처리에 실패했습니다.")}}I()});async function Q(){try{if(t.product=await E(c),t.meta=await C(c),t.reviews=await T(c),await v()||w()){try{await N(c)}catch(e){console.error(e)}try{const e=await O();t.wished=e.some(i=>i.id===c)}catch(e){console.error(e)}}else t.wished=!1;B(),await $(),I()}catch(a){console.error(a),h.root.innerHTML='<p class="empty">상품 데이터를 불러오지 못했습니다.</p>'}}Q();
